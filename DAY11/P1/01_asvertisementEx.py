# 01_advertisementEx.py

# 회귀 분석 : 광고 지출비에 따른 광고 수익 (단순 회귀 분석)

'''
광고 지출비와 광고 수익에 따른 산점도/선 그래프
해당 데이터들이 선형적이라고 가정하고, 가장 이상적인 직선의 방정식을 구하도록 함
(H = wx + b)
'''
...
# 위의 그래프를 보면 이상적인 w에서 비용 함수의 값이 최소가 된다.
# 이 w의 값이 27.5(best)에서 작아지거나 커지면 비용 함수의 값은 커집니다.(비용함수는 U자형 그래프)
# 이것을 그래프로 그려 보겠습니다.
...
# 결론 : x축에 w를 그리고, y축에 비용 함수의 결과 그래프를 확인해보면 이차 함수라는 것을 확인 할 수 있다.
...
print(filename + '파일 저장 됨')
'''
선형 회귀 분석 : 종속 변수 1개와 한개 이상의 독립 변수간의 인과 관계를 선형적으로 분석하는 기법
(가장 일반적으로 많이 사용됨 95%정도)
독립 변수의 개수에 따라서
    단순 회귀 분석 : H = w*x + b
...
머신러닝 에서는 w를 가중치(기울기)라고 하고, b를 bias라고 부른다.
예측값 = 정답(real) = label

비용 함수(cost함수) : 회귀 분석의 성능 지표
    총누적((예측값-실제값)**2)
    가장 이상적인 데이터는 0에 가깝다.

최소 자승법
비용 함수를 구할 때 제곱을 해서 풀어나가므로 이를 최소 제곱법 또는 최소 자승법이라고 한다.
